...
const PROJECTS = [
...

  {
    ...
    specs: {
      'Steering': 'Custom Telescopic Column',
    ...
    mechanical: 'Dual-splined telescopic sleeve with positive quick-lock detents. Static and dynamic torsional shear analysis for 100% torque transmission reliability.',
    hardware: 'Custom-machined steel splined shaft, tubular spaceframe, double wishbone suspension, hydraulic disc brakes.',
    software: 'SOLIDWORKS CAD, ANSYS FEA for frame impact and steering column torque.',
    results: 'FOMBI 2K25 Winners. FOMBI 2K24 Runners-Up. SRKR Gokart Championship Season 1 Runners-Up. Steering withstood full endurance race without deformation.'
  },
  {
    id: 'wall-sander',
    num: '03',
...
/* ——————————————————————————————————————————————————————————————
   CYBERNETIC TEXT SCRAMBLER
—————————————————————————————————————————————————————————————— */
const SCRAMBLE_CHARS = '01#%@&*$_=+<>~█▓░JK';
function scrambleText(el, targetText, duration = 350) {
  if (!el || el.dataset.scrambling === 'true') return;
  el.dataset.scrambling = 'true';
  const orig = targetText || el.textContent;
  const start = performance.now();

  function update(now) {
    const elapsed = now - start;
    const progress = Math.min(1, elapsed / duration);
    const revealIdx = Math.floor(progress * orig.length);

    let output = '';
    for (let i = 0; i < orig.length; i++) {
      if (orig[i] === ' ' || i < revealIdx) {
        output += orig[i];
      } else {
...
function initScrambleEffects() {
  document.querySelectorAll('.projects-heading, .section-label, .about-heading, .dhg-title, .skills-title').forEach(el => {
...
function init3DCardTilts() {
  ...
  const tiltSelectors = '.project-item, .dhg-card, .skill-category, .comp-card, .cert-card, .interest-card';
...
function initTelemetry() {
  ...
  const telScroll = document.getElementById('tel-scroll');
  const telFps = document.getElementById('tel-fps');

  // Mouse coords
  window.addEventListener('mousemove', (e) => {
    if (telPos) telPos.textContent = `X: ${e.clientX} Y: ${e.clientY}`;
  }, { passive: true });
  ...
  window.addEventListener('scroll', () => {
    ...
    const pct = docH > 0 ? Math.round((sTop / docH) * 100) : 0;
    if (telScroll) telScroll.textContent = `${pct}%`;
  }, { passive: true });
  ...
  function calcFPS(now) {
    ...
    if (now - lastTime >= 1000) {
      if (telFps) telFps.textContent = `${frames}`;
      frames = 0;
      lastTime = now;
    }
...
function initThemeSwitcher() {
  ...
  const savedTheme = localStorage.getItem('jk_theme') || 'cyan';
  ...
  function setTheme(t) {
    ...
    localStorage.setItem('jk_theme', t);
    dots.forEach(d => d.classList.toggle('active', d.dataset.themeVal === t));
  }
...
function toggleMatrixRain(forceState) {
  ...
  canvas.classList.toggle('active', matrixRunning);
  ...
  const katakana = '0123456789ABCDEF010101XYZΩλπ';
  const fontSize = 16;
  ...
  function draw() {
    ...
    ctx.fillStyle = 'rgba(5, 7, 17, 0.05)';
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--accent') || '#00c2c7';
...
function initTerminal() {
  ...
  window.addEventListener('keydown', (e) => {
    if ((e.ctrlKey && e.key.toLowerCase() === 'k') || (e.key === '`' && !e.target.matches('input, textarea'))) {
      e.preventDefault();
      const modal = document.getElementById('terminal-modal');
  ...
  const commands = {
    help: () => `
<div style="color:var(--accent); font-weight:700;">AVAILABLE COMMANDS:</div>
  • <span style="color:#ffffff">projects</span>  - List all 8 engineering projects & case studies
  • <span style="color:#ffffff">skills</span>    - View mechanical CAD & robotics capabilities
  • <span style="color:#ffffff">contact</span>   - Get direct email & LinkedIn links
  • <span style="color:#ffffff">theme [name]</span> - Change color (cyan, purple, green, amber, rose)
  • <span style="color:#ffffff">matrix</span>    - Toggle cyberpunk digital background rain
  • <span style="color:#ffffff">clear</span>     - Clear terminal screen
  • <span style="color:#ffffff">exit</span>      - Close this terminal window
`,
    projects: () => `
<div style="color:var(--accent); font-weight:700;">FEATURED ENGINEERING PROJECTS:</div>
1. NIDAR — Autonomous Agriculture Precision Drone (Pixhawk Cube Orange + Jetson Nano)
2. Off-Road ATV & Gokart — Telescopic Steering Mechanism (FOMBI 2K25 Winners)
3. Autonomous Wall Sander Robot — Kinematic Design (Fusion 360)
4. Siemens MCD — Digital Twin Mechatronics Simulation (NX / MCD)
5. ROS 2 & Gazebo — AMR Autonomous Navigation (ESP32 + Micro-ROS)
6. 4WD RC Vehicle — Ackerman Steering Geometry
7. LPG Gas Leakage Safety Shut-Off Automation
8. Multi-Body Dynamic Joint Force Simulation
`,
    skills: () => `
<div style="color:var(--accent); font-weight:700;">TECHNICAL CORE:</div>
• CAD/CAM: Siemens NX, SOLIDWORKS, Fusion 360, Siemens MCD
• CAE/CFD: ANSYS Mechanical FEA, ANSYS Fluent, Gazebo Sim
• Autonomy: ROS 2 Humble, Micro-ROS, ArduPilot, OpenCV
• Embedded: Pixhawk Cube Orange, Jetson Nano, ESP32, Arduino
• Quality: Six Sigma Yellow Belt (DMAIC)
`,
    contact: () => `
<div style="color:var(--accent); font-weight:700;">DIRECT CONTACT:</div>
📧 Email: jaswanthkadali0362@gmail.com
🔗 LinkedIn: https://www.linkedin.com/in/jaswanth-kadali-a969701ba/
🎓 S.R.K.R. Engineering College, Bhimavaram
`,
    matrix: () => {
      toggleMatrixRain();
      return `<div style="color:var(--accent);">Matrix mode: ${matrixRunning ? 'ACTIVATED [ON]' : 'DEACTIVATED [OFF]'}</div>`;
    },
    clear: () => {
  ...
  input?.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
      ...
      if (cmd === 'theme') {
        const theme = args[0];
        if (['cyan', 'purple', 'green', 'amber', 'rose'].includes(theme)) {
          if (theme === 'cyan') document.documentElement.removeAttribute('data-theme');
          else document.documentElement.setAttribute('data-theme', theme);
          localStorage.setItem('jk_theme', theme);
          document.querySelectorAll('.theme-dot').forEach(d => d.classList.toggle('active', d.dataset.themeVal === theme));
          res = `<div style="color:var(--accent);">Theme updated to: ${theme.toUpperCase()}</div>`;
        } else {
          res = `<div style="color:#ef4444;">Unknown theme. Try: theme cyan | purple | green | amber | rose</div>`;
        }
      } else if (commands[cmd]) {
        res = commands[cmd]();
      } else {
        res = `<div style="color:#ef4444;">Command not recognized: "${raw}". Type <span style="color:var(--accent)">help</span> for available commands.</div>`;
      }

      if (output && cmd !== 'clear') {
        output.innerHTML += `
          <div style="margin-top:0.75rem;"><span style="color:var(--accent); font-weight:700;">&gt; ${raw}</span></div>
          <div>${res}</div>
        `;
